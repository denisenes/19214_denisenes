...
main = do
   ...
   let stdout = evalState (runProgram $!! 15000) startState
   putStrLn $! "Output:"
   print $! stdout
   ...
   --backup--
{- runProgram:: Flag -> State ([Int], Register, Register, Register, StdOut, StdIn) String
runProgram 1 = do
                  (_, _, _, _, str, _) <- get
                  return $ (reverse str)
runProgram k = do
            ...
            let acase = aFoo mem d
            case symbol of
               'j' -> put (mem, c, (mem !! d), a, 'j':str, stdin)
               'i' -> put (mem, (mem !! d), d, a, 'i':str, stdin)
               '*' -> put (putInMem mem d acase, c, d, acase, '*':str, stdin)
               'p' -> put (putInMem mem d (crazy a $!! (mem !! d)), c, d, crazy a $! (mem !! d), 'p':str, stdin)
               '<' -> put (mem, c, d, a, ((chr ( 255 .&. a)):str), stdin)
               '/' -> put (mem, c, d, (getSymbol stdin), ('/':str), myTail stdin) --TODO input
               'v' -> put (mem, c, d, -1, 'v':str, stdin)
               'o' -> put (mem, c, d, a, 'o':str, stdin)
               _ -> put (mem, c, d, a, str, stdin)
            (mem, c, d, a, str, stdin) <- get
            put (count mem c, (c+1) `mod` 59049, (d+1) `mod` 59049, a, str, stdin)
            (_, _, _, p, _, _) <- get
            if (p == -1) then runProgram $!! 1 else runProgram $!! (k-1)

         where
               count mem1 c1 = take c1 mem1 ++ [ord $ xlat2 !! ((mem1 !! c1) - 33)] ++ drop (c1 + 1) mem1
               putInMem memx x exp = take x memx ++ [exp] ++ drop (x + 1) memx
               getChar mem1 c1 = xlat1 !! (((mem1 !! c1) - 33 + c1) `mod` 94)
               aFoo mem1 d1 = ((mem1 !! d1) `div` 3) + ((mem1 !! d1) `mod` 3 * 19683)
               --input
               getSymbol [] = 59048
               getSymbol (a:str) = ord a
               --myTail
